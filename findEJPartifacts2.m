function [artifact] = findEJPartifacts2(Vm,time,ind_startEJP,ind_peakEJP,displayArtifact)
...
for i = 1:length(firstEJP)
    ...
    if length(in_burst) > 1
        remove = find(Vm_startEJP(in_burst(2:end)) < Vm_startEJP(firstEJP(i))+0.2)+1;
